...
module.exports = {
  command: 'status',
  description: 'Display MoreBetter installation status and module versions',
  options: [],
  action: async (options) => {
    try {
      ...
      if (!(await fs.pathExists(bmadDir))) {
        await prompts.log.warn('No MoreBetter installation found in the current directory.');
        await prompts.log.message(`Expected location: ${bmadDir}`);
        await prompts.log.message('Run "morebetter install" to set up a new installation.');
        process.exit(0);
        return;
      }
      ...
      if (!manifestData) {
        await prompts.log.warn('No MoreBetter installation manifest found.');
        await prompts.log.message('Run "morebetter install" to set up a new installation.');
        process.exit(0);
        return;
      }
